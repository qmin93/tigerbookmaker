...
| `editorial` | 에디토리얼 매거진 | 비즈니스·전문서·트렌드·인터뷰 | 2단 (본문 + 사이드 인용), serif·sans 혼용, 큰 이미지 |
...
5. `/write` 편집 화면 — 사이드바 토글로 우측 패널 실시간 미리보기
...
5. (선택) `📐 미리보기 ON` 토글 → 우측 패널에 현재 챕터 실시간 렌더링
...
- `[📐 미리보기 ON/OFF]` 토글 버튼
- 토글 ON 시 우측에 split panel — 현재 편집 중인 챕터를 선택된 template의 `Render`로 표시
- 토글 OFF 시 현재 화면 그대로 (편집 영역 100%)
...
- 매거진 2단은 EPUB 뷰어 호환 한계로 EPUB에선 1단 fallback (epubCss에서 처리)
...
|---|---|
| minimal | 그대로 (이미 1단) |
| editorial | 2단 → 1단 fallback. 사이드 인용은 본문 흐름 안에 카드로 |
| classic | 그대로 (이미 1단) |
| practical | 그대로 (이미 1단) |

CSS 미디어 쿼리(`@media (max-width: 768px)`) 또는 Tailwind 반응형 클래스(`md:grid-cols-2`)로 처리.

EPUB은 뷰어 width가 제각각이라 모두 1단 강제.
...
| 3 | `lib/templates/minimal.tsx` (가장 단순, 기준점) | 2h |
| 4 | `lib/templates/classic.tsx` | 2h |
| 5 | `lib/templates/practical.tsx` | 2h |
| 6 | `lib/templates/editorial.tsx` (2단 가장 복잡) | 3h |
| 7 | 사이드바 `<TemplateSelector />` UI + 자동 매칭 호출 | 4h |
| 8 | 미리보기 panel + 토글 + 실시간 렌더링 | 8h |
| 9 | `/share/[id]` template 분기 적용 | 2h |
| 10 | `/book/[id]` template 적용 (책 미리보기 영역) | 2h |
| 11 | EPUB export — epubCss 통합 | 4h |
| 12 | PDF export — pdfHtmlWrapper 통합 | 4h |
| 13 | Cover prompt에 styleHint 추가 + 검증 | 2h |
...
| 미리보기 panel이 무거워서 /write 느려짐 | 중 | 중 | 미리보기 OFF 기본 + lazy render + debounce |
| 매거진 2단 한국어 줄바꿈 어색 | 중 | 낮 | 컬럼 폭 조절 + word-break-keep-all CSS |
