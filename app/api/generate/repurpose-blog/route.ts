...
// 가격 정책 (Sang-nim 10x 인상, 2026-05): 블로그 시리즈 ₩1,500 고정
const FIXED_COST_KRW = 1500;
const MIN_BALANCE_KRW = 1500;
  ...
  try {
    ...
    if (user.balance_krw < MIN_BALANCE_KRW) {
      return NextResponse.json({
        ...
        message: `잔액 부족 (블로그 시리즈 ₩${FIXED_COST_KRW.toLocaleString()} 필요)`,
